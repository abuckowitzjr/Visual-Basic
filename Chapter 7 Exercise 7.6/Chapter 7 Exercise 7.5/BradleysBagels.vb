...
Public Class frmBradleysBagels
    Private Sub btnAddBagelType_Click(sender As System.Object, e As System.EventArgs) Handles btnAddBagelType.Click
        'verifies if item entered already exists in the list, if so, displays messagebox informing so, if not, adds entered item.
        ...
        Else
            BagelTypeEntered = comboboxBagelTypes.Text.ToUpper
            Do Until blnItemFound Or intItemIndex = comboboxBagelTypes.Items.Count
                BagelTypeList = CStr(comboboxBagelTypes.Items(intItemIndex)).ToUpper
                If BagelTypeEntered = BagelTypeList Then
                    MessageBox.Show("Bagel type is already in the list.", "Input Error", MessageBoxButtons.OK)
